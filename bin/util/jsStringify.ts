...
export const jsStringify = (
	...
	const replacer = options.replacer || ( options.replacer = ( obj, level = 0, curLength = 0 ): string => {
		...
		if ( typeof obj.toJS === "function" )
			return obj.toJS( obj, space as string, level, curLength, options );
		...
		if ( Array.isArray( obj ) )
			return stringifyArray( obj, space as string, level, curLength, spaceWidth, cleanedOptions );

		return stringifyObject( obj, space as string, level, curLength, spaceWidth, cleanedOptions );
